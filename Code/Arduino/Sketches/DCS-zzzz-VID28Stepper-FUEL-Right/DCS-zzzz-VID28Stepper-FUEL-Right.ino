...
Vid29Stepper apu(0x10cc, stepper, stepperConfig, [](unsigned int newValue) -> unsigned int {
  /* this function needs to map newValue to the correct number of steps */
  return map(newValue, 0, 65535, 0, stepperConfig.maxSteps);
}); 
DcsBios::Switch2Pos fqisTest("FQIS_TEST", 6);

const byte fqisSelectPins[5] = {7, 8, 9, 10, 11};
DcsBios::SwitchMultiPos fqisSelect("FQIS_SELECT", fqisSelectPins, 5);

unsigned int g_iCounter = 0;
void PollAllControls()
{
  fqisSelect.pollInputCurrent();
}
